...
bool CUILoading::Load(HANDLE hFile)
{
	...
	__ASSERT(m_pText_Version, "m_pText_Version NULL UI Component!!");
	if(m_pText_Version)
	{
		char szVersion[128];
	...
	__ASSERT(m_pText_Info, "m_pText_Info NULL UI Component!!");
	
	m_pProgress_Loading = (CN3UIProgress*)(CN3UIBase::GetChildByID("Progress_Loading"));
	__ASSERT(m_pProgress_Loading, "m_pProgress_Loading NULL UI Component!!");

	this->SetPosCenter();
	m_pText_Version->SetPos(10, 10);
...
void CUILoading::Render(const std::string& szInfo, int iPercentage)
{
	...
	CGameProcedure::s_pEng->Clear(crEnv);
