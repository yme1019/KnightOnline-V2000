...
bool CUIPartyBBS::Load(HANDLE hFile)
{
	...
	__ASSERT(m_pBtn_Register, "m_pBtn_Register NULL UI Component!!!");

	m_pBtn_RegisterCancel = (CN3UIButton*)(this->GetChildByID("btn_delete"));			
	__ASSERT(m_pBtn_RegisterCancel, "m_pBtn_RegisterCancel NULL UI Component!!!");

	m_pBtn_Whisper = (CN3UIButton*)(this->GetChildByID("btn_whisper"));					
	__ASSERT(m_pBtn_Whisper, "m_pBtn_Whisper NULL UI Component!!!");

	m_pBtn_Party = (CN3UIButton*)(this->GetChildByID("btn_Party"));						
	__ASSERT(m_pBtn_Party, "m_pBtn_Party NULL UI Component!!!");

	m_pText_Page = (CN3UIString*)(this->GetChildByID("string_page"));					
	__ASSERT(m_pText_Page, "m_pText_Page NULL UI Component!!!");
...
bool CUIPartyBBS::ReceiveMessage(CN3UIBase* pSender, uint32_t dwMsg)
{
	...
	if( dwMsg == UIMSG_BUTTON_CLICK )
	{
		...
		else if(pSender == m_pBtn_Register)
		{
			...
			CGameProcedure::MessageBoxPost(szMsg, "", MB_YESNO, BEHAVIOR_PARTY_BBS_REGISTER);
		}
		else if(pSender == m_pBtn_RegisterCancel)
		{
			std::string szMsg;
			::_LoadStringFromResource(IDS_PARTY_BBS_REGISTER_CANCEL, szMsg);
			CGameProcedure::MessageBoxPost(szMsg, "", MB_YESNO, BEHAVIOR_PARTY_BBS_REGISTER_CANCEL); 
		}
...
{
	if(m_pText_Page) m_pText_Page->SetStringAsInt(m_iCurPage+1);
...
void CUIPartyBBS::PartyStringSet(uint8_t byType)
{
	...
	if(CGameProcedure::s_pPlayer->m_bRecruitParty)
	{
		...
		CGameProcedure::s_pProcMain->MsgSend_StateChange(N3_SP_STATE_CHANGE_RECRUIT_PARTY, 0x02);
	}
	else
	{
		CGameProcedure::s_pPlayer->InfoStringSet("", 0);
		CGameProcedure::s_pProcMain->MsgSend_StateChange(N3_SP_STATE_CHANGE_RECRUIT_PARTY, 0x01);
	}
}
