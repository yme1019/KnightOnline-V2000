...
void CUIWareHouseDlg::Render()
{
	...
	for(UIListReverseItor itor = m_Children.rbegin(); m_Children.rend() != itor; ++itor)
	{
		...
		if ((GetState() == UI_STATE_ICON_MOVING) && (pChild->UIType() == UI_TYPE_ICON) && (CN3UIWndBase::m_sSelectedIconInfo.pItemSelect) && ((CN3UIIcon *)pChild == CN3UIWndBase::m_sSelectedIconInfo.pItemSelect->pUIIcon) )	continue;
			pChild->Render();
	...
	for( i = 0; i < MAX_ITEM_TRADE; i++ )
	{
		if (m_pMyWare[m_iCurPage][i] && ((m_pMyWare[m_iCurPage][i]->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE) || (m_pMyWare[m_iCurPage][i]->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE_SMALL)))
		{
...
{
	_TBL_TABLE_UI* pTbl = CGameBase::file_Tbl_UI.Find(CGameBase::s_pPlayer->m_InfoBase.eNation);
...
uint32_t CUIWareHouseDlg::MouseProc(uint32_t dwFlags, const POINT& ptCur, const POINT& ptOld)
{
	...
	if ( (GetState() == UI_STATE_ICON_MOVING) && (CN3UIWndBase::m_sSelectedIconInfo.UIWndSelect.UIWnd == UIWND_WARE_HOUSE) )
	{
...
#define FAIL_RETURN {	\
	...
	case UIWND_DISTRICT_TRADE_NPC:
		if (eUIWnd == UIWND_DISTRICT_TRADE_MY)
		{
				...
				(CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE_SMALL))
			{
				...
				for (i = 0; i < MAX_ITEM_INVENTORY; i++)
				{
					if ((m_pMyWareInv[i]) && (m_pMyWareInv[i]->pItemBasic->dwID == CN3UIWndBase::m_sSelectedIconInfo.pItemSelect->pItemBasic->dwID) &&
						(m_pMyWareInv[i]->pItemExt->ItemIndex == CN3UIWndBase::m_sSelectedIconInfo.pItemSelect->pItemExt->ItemIndex))
					{
						bFound = true;
						iDestiOrder = i;
			...
			else
			{
				...
				SendToServerFromWareMsg(CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemBasic->dwID +
					CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemExt->ItemIndex, m_iCurPage,
		...
		else
		{
			...
			CN3UIWndBase::m_sRecoveryJobInfo.UIWndSourceEnd.iOrder = iDestiOrder;
			if (m_pMyWare[m_iCurPage][iDestiOrder])
			{
				CN3UIWndBase::m_sRecoveryJobInfo.m_bWaitFromServer = false;
				CN3UIWndBase::m_sRecoveryJobInfo.pItemSource = NULL;
				CN3UIWndBase::m_sRecoveryJobInfo.pItemTarget = NULL;
		...
		if (eUIWnd == UIWND_DISTRICT_TRADE_NPC)	
		{
				...
				for (int iPage = 0; iPage < MAX_ITEM_WARE_PAGE; iPage++)
				{
					...
					for (i = 0; i < MAX_ITEM_TRADE; i++)
					{
						if ((m_pMyWare[iPage][i]) && (m_pMyWare[iPage][i]->pItemBasic->dwID == CN3UIWndBase::m_sSelectedIconInfo.pItemSelect->pItemBasic->dwID) &&
							(m_pMyWare[iPage][i]->pItemExt->ItemIndex == CN3UIWndBase::m_sSelectedIconInfo.pItemSelect->pItemExt->ItemIndex))
						{
							bFound = true;
							iDestiOrder = i;
							CN3UIWndBase::m_sRecoveryJobInfo.UIWndSourceEnd.iOrder = iDestiOrder;
			...
			else
			{
				...
				SendToServerToWareMsg(CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemBasic->dwID +
					CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemExt->ItemIndex,
...
void CUIWareHouseDlg::ReceiveResultToWareMsg(uint8_t bResult)
{
	...
	if (bResult != 0x01)
	{
		...
		{
			m_pMyWareInv[CN3UIWndBase::m_sRecoveryJobInfo.UIWndSourceStart.iOrder] = m_pMyWare[m_iCurPage][CN3UIWndBase::m_sRecoveryJobInfo.UIWndSourceEnd.iOrder];
	...
	else
	{
		...
		if (((CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE) ||
			(CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE_SMALL)) &&
			!m_pMyWareInv[CN3UIWndBase::m_sRecoveryJobInfo.UIWndSourceStart.iOrder]->pUIIcon->IsVisible())
		{
...
void CUIWareHouseDlg::ReceiveResultFromWareMsg(uint8_t bResult)
{
	...
	else
	{
		...
		if(((CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE) || 
			(CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE_SMALL) ) && !m_pMyWare[m_iCurPage]
			[CN3UIWndBase::m_sRecoveryJobInfo.UIWndSourceStart.iOrder]->pUIIcon->IsVisible())
		{				
...
void CUIWareHouseDlg::AddItemInWare(int iItem, int iDurability, int iCount, int iIndex)
{
	...
	if(pItem && pItem->byExtIndex >= 0 && pItem->byExtIndex < MAX_ITEM_EXTENSION) pItemExt = CGameBase::s_pTbl_Items_Exts[pItem->byExtIndex].Find(iItem%1000);	
...
bool CUIWareHouseDlg::Load(HANDLE hFile)
{
	...
	__ASSERT(m_pBtnGold, "m_pBtnGold NULL UI Component!!");

	m_pBtnGoldWareHouse = (CN3UIButton*)(this->GetChildByID("btn_gold_warehouse"));	
	__ASSERT(m_pBtnGoldWareHouse, "m_pBtnGoldWareHouse NULL UI Component!!");

	m_pBtnClose			= (CN3UIButton*)(this->GetChildByID("btn_close"));			
	__ASSERT(m_pBtnClose, "m_pBtnClose NULL UI Component!!");

	m_pBtnPageUp		= (CN3UIButton*)(this->GetChildByID("btn_page_up"));		
	__ASSERT(m_pBtnPageUp, "m_pBtnPageUp NULL UI Component!!");

	m_pBtnPageDown		= (CN3UIButton*)(this->GetChildByID("btn_page_down"));		
	__ASSERT(m_pBtnPageDown, "m_pBtnPageDown NULL UI Component!!");
