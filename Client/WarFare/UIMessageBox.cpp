	...
	if (dwMsg == UIMSG_BUTTON_CLICK)
	{
		...
		if (pSender == m_pBtn_OK || pSender == m_pBtn_Yes)
		{
			...
			switch(m_eBehavior)
			{
				case BEHAVIOR_EXIT :
					{
						if(CGameProcedure::s_pProcActive == pProcMain) 
						{
				...
				case BEHAVIOR_FORCE_DISBAND :			
					pProcMain->MsgSend_PartyOrForceLeave(1); break;
				case BEHAVIOR_REQUEST_BINDPOINT :		
					if(pShape) pProcMain->MsgSend_ObjectEvent(pShape->m_iEventID, 0); 	
					break;
				...
				case BEHAVIOR_PARTY_BBS_REGISTER:			
					if(pProcMain->m_pUIPartyBBS)pProcMain->m_pUIPartyBBS->MsgSend_Register(); 
					break;
				case BEHAVIOR_PARTY_BBS_REGISTER_CANCEL:	
					if(pProcMain->m_pUIPartyBBS)pProcMain->m_pUIPartyBBS->MsgSend_RegisterCancel(); 
					break;
		...
		else if (pSender == m_pBtn_No || pSender == m_pBtn_Cancel)
		{
			...
			switch(m_eBehavior)
			{
			case BEHAVIOR_PARTY_PERMIT:				pProcMain->MsgSend_PartyOrForcePermit(0, false); 
				break;
			case BEHAVIOR_FORCE_PERMIT:				pProcMain->MsgSend_PartyOrForcePermit(1, false); 
				break;
			case BEHAVIOR_PERSONAL_TRADE_PERMIT :	pProcMain->m_pSubProcPerTrade->LeavePerTradeState(PER_TRADE_RESULT_MY_DISAGREE);	
				break;
			case BEHAVIOR_PERSONAL_TRADE_FMT_WAIT:	pProcMain->m_pSubProcPerTrade->LeavePerTradeState(PER_TRADE_RESULT_MY_CANCEL);	
				break;
