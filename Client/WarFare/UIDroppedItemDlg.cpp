...
void CUIDroppedItemDlg::Render()
{
	...
	for( int i = 0; i < MAX_ITEM_BUNDLE_DROP_PIECE; i++ )
	{
		if ( m_pMyDroppedItem[i] && ( (m_pMyDroppedItem[i]->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE) || (m_pMyDroppedItem[i]->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE_SMALL) ) )
		{
...
void CUIDroppedItemDlg::AddToItemTable(int iItemID, int iItemCount, int iOrder)
{
	...
	printf("Dropped item from server to ItemDlg %d \n", iItemID);
...
#define FAIL_CODE {		\
	...
	switch (dwMsg & dwBitMask)
	{
		...
		case UIMSG_ICON_UP:
			...
			eType = CGameProcedure::MakeResrcFileNameForUPC(pItem, NULL, &szIconFN, ePart, ePlug);
...
void CUIDroppedItemDlg::GetItemByIDToInventory(uint8_t bResult, int iItemID, int iGold, int iPos, int iItemCount, int iStrLen, std::string strString)
{
	bool				bFound = false;
	__TABLE_ITEM_BASIC*	pItem = NULL;
	__TABLE_ITEM_EXT*	pItemExt = NULL;
	...
	if (bResult == 0x03)
	{
		...
		for( i = 0; i < MAX_ITEM_BUNDLE_DROP_PIECE; i++ )
		{
			if( (m_pMyDroppedItem[i]) && (m_pMyDroppedItem[i]->pItemBasic->dwID+m_pMyDroppedItem[i]->pItemExt->ItemIndex == iItemID) )
			{
				bFound = true;
				break;
			}
	...
	if (bResult == 0x05)
	{
		...
		if (spItemDest)	
		{
			if ( iItemID != (spItemDest->pItemBasic->dwID+spItemDest->pItemExt->ItemIndex) )
			{
