...
void CUITransactionDlg::Render()
{
	...
	for( i = 0; i < MAX_ITEM_INVENTORY; i++ )
	{
		if ( m_pMyTradeInv[i] && ( (m_pMyTradeInv[i]->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE) || (m_pMyTradeInv[i]->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE_SMALL) ) )
		{
...
void CUITransactionDlg::InitIconWnd(e_UIWND eWnd)
{
	...
	__ASSERT(m_pStrMyGold, "m_pStrMyGold NULL UI Component!!");
	if(m_pStrMyGold) m_pStrMyGold->SetString("0");

	m_pUIInn		= (CN3UIImage*)GetChildByID("img_inn");			
	__ASSERT(m_pUIInn, "m_pUIInn NULL UI Component!!");

	m_pUIBlackSmith = (CN3UIImage*)GetChildByID("img_blacksmith");	
	__ASSERT(m_pUIBlackSmith, "m_pUIBlackSmith NULL UI Component!!");

	m_pUIStore		= (CN3UIImage*)GetChildByID("img_store");		
	__ASSERT(m_pUIStore, "m_pUIStore NULL UI Component!!");
}
...
void CUITransactionDlg::ItemCountOK()
{
	...
	switch (CN3UIWndBase::m_pCountableItemEdit->GetCallerWndDistrict())
	{
		case UIWND_DISTRICT_TRADE_NPC:
			...
			if ( m_pMyTradeInv[CN3UIWndBase::m_sRecoveryJobInfo.UIWndSourceEnd.iOrder] )
			{
...
#define FAIL_RETURN {	\
	...
	if ((CN3UIWndBase::m_sSelectedIconInfo.UIWndSelect.UIWndDistrict != UIWND_DISTRICT_TRADE_NPC) && (CN3UIWndBase::m_sSelectedIconInfo.UIWndSelect.UIWndDistrict != UIWND_DISTRICT_TRADE_MY) )
		FAIL_RETURN
			...
			if (eUIWnd == UIWND_DISTRICT_TRADE_MY)		// 사는 경우..
			{
					...
					(CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE_SMALL) )
				{
					...
					for( i = 0; i < MAX_ITEM_INVENTORY; i++ )
					{
						...
						if( (m_pMyTradeInv[i]) && (m_pMyTradeInv[i]->pItemBasic->dwID == CN3UIWndBase::m_sSelectedIconInfo.pItemSelect->pItemBasic->dwID) && (m_pMyTradeInv[i]->pItemExt->ItemIndex == CN3UIWndBase::m_sSelectedIconInfo.pItemSelect->pItemExt->ItemIndex) )
						{
							bFound = true;
							iDestiOrder = i;
						}
				...
				else
				{
					...
					SendToServerBuyMsg(CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemBasic->dwID+
						CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->pItemExt->ItemIndex, iDestiOrder,
						CN3UIWndBase::m_sRecoveryJobInfo.pItemSource->iCount);

					std::string szIconFN;
					e_PartPosition ePart;
					e_PlugPosition ePlug;
					CGameProcedure::MakeResrcFileNameForUPC(m_pMyTrade[m_iCurPage][CN3UIWndBase::m_sRecoveryJobInfo.UIWndSourceStart.iOrder]->pItemBasic, NULL, &szIconFN, ePart, ePlug);
