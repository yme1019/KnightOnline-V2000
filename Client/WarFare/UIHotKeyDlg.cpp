...
//	uint32_t bitMask;
	...
	while(iHCount--)
	{
		...
		if( CGameProcedure::RegGetSetting(str.c_str(), &HD, sizeof(CHotkeyData)) )
		{
			...
			pUSkill = CGameBase::file_Tbl_Skill.Find(HD.iID);
			if ( !pUSkill )
				continue;
	...
	for( i = 0; i < MAX_SKILL_HOTKEY_PAGE; i++ )
	{
		for( j = 0; j < MAX_SKILL_IN_HOTKEY; j++ )
		{
			if ( m_pMyHotkey[i][j] != NULL )
			{
				...
				CHotkeyData HD(i, j, m_pMyHotkey[i][j]->pSkill->SkllNum);
...
bool CUIHotKeyDlg::ReceiveIconDrop(__IconItemSkill* spItem, POINT ptCur)
{
	...
	else
	{
		...
		_TB_TABLE_UPC_SKILL* pUSkill = CGameBase::file_Tbl_Skill.Find(spItem->pItemBasic->dwEffectID1);
		if ( pUSkill == NULL ) 
			return false;

		if ( pUSkill->SkllNum < UIITEM_TYPE_SONGPYUN_ID_MIN)
			return false;
