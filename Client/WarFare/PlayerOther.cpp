...
bool CPlayerOther::Init(e_Race eRace, int iFace, int iHair, uint32_t* pdwItemIDs, int* piItenDurabilities)
{
	...
	__TABLE_PLAYER_LOOKS* pLooks = s_pTbl_UPC_Looks.Find(eRace);	
	if(NULL == pLooks) 
	{
...
//			else if(7 == i) { ePlug = PLUG_POS_LEFTHAND; }
		...
		{
			pItem = s_pTbl_Items_Basic.Find(pdwItemIDs[i]/1000*1000);	
			...
			e_ItemType eType = CGameProcedure::MakeResrcFileNameForUPC(pItem, &szFN, NULL, ePart, ePlug, m_InfoBase.eRace); 
		...
		else if(PART_POS_HAIR_HELMET == ePart)
		{
			this->PartSet(ePart, szFN, pItem, pItemExt);
		}
		else if(5 == i) 
	...
	if(pPartHairHelmet->FileName().empty()) 
		this->InitHair();
...
void CPlayerOther::InitFace()
{
	...
	if(pItem && !pItem->szPartFNs[PART_POS_FACE].empty()) 
	{
...
{
	__TABLE_PLAYER_LOOKS* pItem = s_pTbl_UPC_Looks.Find(m_InfoBase.eRace);
	if(pItem && !pItem->szPartFNs[PART_POS_HAIR_HELMET].empty()) 
	{
		char szBuff[256] = "", szDir[128] = "", szFName[128] = "", szExt[16] = "";
