...
public:
	...
	uint32_t				m_dwRegionMagicState;	
	_TB_TABLE_UPC_SKILL		m_dwRegionSkill;

	//related non-casting action magic...
	uint32_t				m_dwCastingStateNonAction;
...
protected:
	bool	CheckValidCondition(int iTargetID, _TB_TABLE_UPC_SKILL* pSkill);
	bool	CheckValidDistance(_TB_TABLE_UPC_SKILL* pSkill, __Vector3 vTargetPos, float fTargetRadius);
	
	void	InitType4();

	void	StartSkillMagicAtTargetPacket(_TB_TABLE_UPC_SKILL* pSkill, int16_t TargetID);
	void	StartSkillMagicAtPosPacket(_TB_TABLE_UPC_SKILL* pSkill, __Vector3 vPos);
	
	void	ProcessCasting();
	void	ProcessCombo();

	void	MobCasting(_TB_TABLE_UPC_SKILL* pSkill, int iSourceID);

	void	FlyingType2(_TB_TABLE_UPC_SKILL* pSkill, int iSourceID, int iTargetID, int16_t* pData);
	...
	int		AddIdx(uint32_t dwMagicID, int iNum=1);
	void	RemoveIdx(int idx);

	void	SuccessCast(_TB_TABLE_UPC_SKILL* pSkill, CPlayerBase* pTarget);
	void	FailCast(_TB_TABLE_UPC_SKILL* pSkill);
	
public:
	bool	CheckValidSkillMagic(_TB_TABLE_UPC_SKILL* pSkill);
	void	StunMySelf(__TABLE_UPC_SKILL_TYPE_3* pType3);
	void	StopCastingByRatio();
	void	ClearDurationalMagic();
	D3DCOLOR	TraceColorGet(_TB_TABLE_UPC_SKILL* pSkill);

	bool	IsPositiveMagic(uint32_t dwMagicID);
	bool	IsCasting();
	uint32_t	GetMagicID(int idx);
		
	bool	MsgSend_MagicProcess(int iTargetID, _TB_TABLE_UPC_SKILL* pSkill);
