...
void CGameProcMain::Init()
{
	...
	if(m_pSnd_Battle==NULL)
	{
		...
		m_pSnd_Battle = s_pEng->s_SndMgr.CreateStreamObj(iIDSndBattle);	
		if(m_pSnd_Battle) 
		{
	...
	if(m_pSnd_Town==NULL)
	{
		m_pSnd_Town = s_pEng->s_SndMgr.CreateStreamObj(ID_SOUND_BGM_TOWN);
		if(m_pSnd_Town)
		{
...
void CGameProcMain::Tick()
{
	...
	this->ProcessLocalInput(dwMouseFlags);					
...
bool CGameProcMain::ProcessPacket(Packet& pkt)
{
	...
	case WIZ_TEST_PACKET:
		...
		case WIZ_WARP:
			{
				...
				s_pFX->TriggerBundle(s_pPlayer->IDNumber(),-1,s_pPlayer->m_InfoBase.eNation == NATION_KARUS? FXID_WARP_KARUS: FXID_WARP_ELMORAD,s_pPlayer->IDNumber(),-1,0);
			}
			return true;
		case WIZ_MOVE:
		...
		case WIZ_REGENE:
			{
				...
				std::string szMsg = "Press OK to teleport back to the re-spawn point.";
		...
		case WIZ_CONCURRENTUSER: 
			{
				int iUserCount = pkt.read<int16_t>();
				std::string szFmt;
...
void CGameProcMain::ProcessLocalInput(uint32_t dwMouseFlags)
{
	...
	{
		float fPitch = 0;
		if(s_pLocalInput->IsKeyDown(SDL_SCANCODE_HOME)) fPitch = D3DXToRadian(45.0f);		
		else if(s_pLocalInput->IsKeyDown(SDL_SCANCODE_END)) fPitch = D3DXToRadian(-45.0f);
...
void CGameProcMain::MsgSend_Weather(int iWeather, int iPercent)
{
	...
	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_WEATHER); 
	CAPISocket::MP_AddByte(byBuff, iOffset, iWeather); 
	CAPISocket::MP_AddShort(byBuff, iOffset, (int16_t)iPercent);
...
void CGameProcMain::MsgRecv_MyInfo_RealmPoint(Packet& pkt)
{
	...
	if (bType == 1) 
	{
		...
		{
			char szBuf[128]; 
			std::string szFmt; ::_LoadStringFromResource(IDS_LOYALTY_CHANGE_GET, szFmt);
		...
		{
			char szBuf[128]; 
			std::string szFmt; ::_LoadStringFromResource(IDS_LOYALTY_CHANGE_LOST, szFmt);
			sprintf(szBuf, szFmt.c_str(), -iLoyaltyDelta);
			MsgOutput(szBuf, 0xffff3b3b);
		}
		...
		if (m_pUIVar->m_pPageState)
			m_pUIVar->m_pPageState->UpdateRealmPoint(s_pPlayer->m_InfoExt.iRealmPoint, s_pPlayer->m_InfoExt.iRealmPointMonthly);
	}
	else if (bType == 2)
	{
...
void CGameProcMain::InitUI()
{
	...
	m_pUICmd->LoadFromFile(pTbl->Co_Cmd_us);
	...
	m_pUIChatDlg->LoadFromFile(pTbl->co_chatting_box_us);
	...
	m_pUIChatDlg2->LoadFromFile(pTbl->Co_chat_small_us);
	...
	m_pUIMsgDlg->LoadFromFile(pTbl->co_information_box_us);
	...
	m_pUIMsgDlg2->LoadFromFile(pTbl->Co_msgoutput_small_us);
	...
	m_pUIStateBarAndMiniMap->LoadFromFile(pTbl->Co_StateBar_us);
	...
	m_pUIVar->LoadFromFile(pTbl->Co_Various_frame_us);
	m_pUIVar->SetVisibleWithNoSound(false);
	m_pUIVar->m_pPageState->LoadFromFile(pTbl->Co_Page_State_us);
	m_pUIVar->m_pPageState->SetVisibleWithNoSound(true);
	m_pUIVar->m_pPageKnights->LoadFromFile(pTbl->Co_Page_clan_us);
	m_pUIVar->m_pPageKnights->SetVisibleWithNoSound(false);
	m_pUIVar->m_pPageFriends->LoadFromFile(pTbl->Co_Page_Friends_us);
	...
	m_pUIVar->m_pPageQuest->LoadFromFile(pTbl->Co_Page_Quest_us);
	m_pUIVar->m_pPageQuest->SetVisibleWithNoSound(false);
	
	m_pUITargetBar->Init(s_pUIMgr);
	m_pUITargetBar->LoadFromFile(pTbl->co_TargetBar_us);
	...
	m_pUIHelp->LoadFromFile(pTbl->Co_Help_us);
...
//	m_pUIHelp->SetStyle(UISTYLE_SHOW_ME_ALONE);
	...
	m_pUINotice->LoadFromFile(pTbl->Co_Notice_us);
	m_pUINotice->SetVisibleWithNoSound(false);

	m_pUIClassChange->Init(s_pUIMgr);
	m_pUIClassChange->LoadFromFile(pTbl->co_changeclass_us);
	...
	m_pUINpcChange->LoadFromFile(pTbl->co_change_us);
	...
	m_pUINpcEvent->LoadFromFile(pTbl->co_smith_us);
	...
	m_pUINpcTalk->LoadFromFile(pTbl->co_talk_us);
	...
	m_pUIWarp->LoadFromFile(pTbl->Co_Warp_us);
...
//	m_pUITradeList->SetPos(iX, iY);
	...
	m_pUIItemREDlg->LoadFromFile(pTbl->Co_change_us);
	...
	m_pUIRepairTooltip->LoadFromFile(pTbl->Co_tooltop_repair_us);
	...
	m_pUIPartyOrForce->LoadFromFile(pTbl->Co_Party_us);
...
//	CGameProcedure::UIPostData_Read(UI_POST_WND_PARTY, m_pUIPartyOrForce, iX, 0);
	...
	m_pUIDroppedItemDlg->LoadFromFile(pTbl->co_DroppedItem_us);
	...
	m_pUITransactionDlg->LoadFromFile(pTbl->Co_Transaction_us);
	...
	m_pUIInventory->LoadFromFile(pTbl->Co_Inventory_us);
	...
	CN3UIWndBase::m_pCountableItemEdit->LoadFromFile(pTbl->Co_PersonalTradeEdit_us);
	...
	m_pUISkillTreeDlg->LoadFromFile(pTbl->Co_SkillTree_us);
	...
	m_pUICmdListDlg->LoadFromFile(pTbl->Co_CmdList_us);
	...
	m_pUICmdEditDlg->LoadFromFile(pTbl->co_CmdEdit_us);
	...
	m_pUIHotKeyDlg->LoadFromFile(pTbl->co_HotKey_us);
	...
	m_pUIKnightsOp->LoadFromFile(pTbl->co_KnightsOperation_us);
	m_pUIKnightsOp->SetVisibleWithNoSound(false);

	m_pUIPartyBBS->Init(s_pUIMgr);
	m_pUIPartyBBS->LoadFromFile(pTbl->Co_PartyBoard_us);//, N3FORMAT_VER_1298);
	...
	m_pUIWareHouseDlg->LoadFromFile(pTbl->Co_WareHouse_us);
	...
	m_pTargetSymbol->LoadFromFile(pTbl->Co_TargetSymbol); // 플레이어가 타겟으로 잡은 캐릭터의 위치위에 그리면 된다..

	m_pUIInn->Init(s_pUIMgr);
	m_pUIInn->LoadFromFile(pTbl->co_inn_us);
	...
	m_pUICreateClanName->LoadFromFile(pTbl->co_creat_clan_us);
	...
	m_pUIDead->LoadFromFile(pTbl->co_Dead_us);
	...
	m_pUITradeBBS->LoadFromFile(pTbl->Co_saleboard_us);
	...
	m_pUITradeBBSSelector->LoadFromFile(pTbl->co_saleboardselection_us);
	...
	// 상거래 항목 내용
	m_pUITradeBBSEdit->LoadFromFile(pTbl->co_saleboardmemo_us);
...
void CGameProcMain::InitZone(int iZone, const __Vector3& vPosPlayer)
{
	...
	if(iZonePrev != iZone) // 다른 존일 경우에만 로딩..
	{
		...
		CLogWriter::Write("%d->Find(s_pPlayer->m_InfoExt.iZoneCur)", file_Tbl_Zones); // TmpLog1122
		_TBL_TABLE_ZONE* pZoneData = file_Tbl_Zones.Find(s_pPlayer->m_InfoExt.iZoneCur);
		if(NULL == pZoneData) {
...
void CGameProcMain::MsgSend_StateChange(e_SubPacket_State eSP, int iState)
{
	...
	CAPISocket::MP_AddByte(byBuff, iOffset, eSP);
	CAPISocket::MP_AddShort(byBuff, iOffset, iState);
	...
	if(pTarget && pTarget->m_InfoBase.iAuthority != AUTHORITY_MANAGER)
	{
		...
		if(iIDTargetPrev != s_pPlayer->m_iIDTarget) 
		{
			...
			if( pTarget->IsAlive() )
			{
				...
				this->MsgSend_RequestTargetHP(s_pPlayer->m_iIDTarget, 0x01);
			}
			else
			{
				m_pUITargetBar->SetVisible(false);
...
void CGameProcMain::MsgRecv_ObjectEvent(Packet& pkt)
{
	...
	else if(OBJECT_TYPE_DOOR_LEFTRIGHT == iType || OBJECT_TYPE_DOOR_TOPDOWN == iType || OBJECT_TYPE_LEVER_TOPDOWN == iType || OBJECT_TYPE_FLAG == iType) 
	{
		int iID = pkt.read<int16_t>();
...
void CGameProcMain::ParseChattingCommand(const std::string& szCmd)
{
	...
	switch(eCmd)
	{
		...
		case CMD_TRADE:
		{
			...
			if(	pOPC && (pOPC->Position() - s_pPlayer->Position()).Magnitude() < (pOPC->Height() + 5.0f) && !m_pUITransactionDlg->IsVisible())
			{
		...
		case CMD_PARTY:
		{
			...
			{
				std::string szMsg;
				if (this->MsgSend_PartyOrForceCreate(0, pTarget->IDString())) ::_LoadStringFromResource(IDS_PARTY_INVITE, szMsg);
				else
		...
		case CMD_NOTICE:
		{
			if(szCmd.size() >= (s_szCmdMsg[CMD_NOTICE].size()+2))
			{
...
void CGameProcMain::UpdateUI_MiniMap()
{
	...
	for(; it != itEnd; it++) // NPC
	{
		...
		if(eNation != pNPC->m_InfoBase.eNation) crType = 0xff800000; 
		else crType = 0xff00a0ff; 
	...
	CPlayerOther* pUPC = NULL;
	_TBL_TABLE_ZONE* pZoneInfo = file_Tbl_Zones.Find(s_pPlayer->m_InfoExt.iZoneCur);
...
void CGameProcMain::UpdateUI_TargetBar()
{
	...
	if(pTarget) 
	{
		...
		if(fTimeTmp > m_pUITargetBar->m_fTimeSendPacketLast + PACKET_INTERVAL_REQUEST_TARGET_HP)
		{
...
void CGameProcMain::UpdateCameraAndLight()
{
	...
	s_pEng->Tick(crDiffuses, crAmbients, ACT_WORLD->GetFogColorWithSky(),  vPosPlayer, s_pPlayer->Rotation(), s_pPlayer->Height(), ACT_WORLD->GetSunAngleByRadinWithSky());
	s_pEng->ApplyCameraAndLight();	
}
...
{
	e_SubPacket_KnightsList eSP = (e_SubPacket_KnightsList)(pkt.read<uint8_t>());
	switch(eSP)
	{
...
void CGameProcMain::MsgRecv_NpcChangeOpen(Packet& pkt)
{
	uint8_t	bSubCom = pkt.read<uint8_t>();
...
void CGameProcMain::MsgRecv_AllPointInit(Packet& pkt)
{
	uint8_t	bType		= pkt.read<uint8_t>();	
...
void CGameProcMain::MsgRecv_SkillPointInit(Packet& pkt)
{
	uint8_t	bType		= pkt.read<uint8_t>();	
	uint32_t	dwGold		= pkt.read<uint32_t>();	
	int i;
...
bool CGameProcMain::OnMouseLBtnPress(POINT ptCur, POINT ptPrev)
{
	...
	POINT ptPlayer = ::_Convert3D_To_2DCoordinate(s_pPlayer->Position(),  CN3Base::s_CameraData.mtxView, CN3Base::s_CameraData.mtxProjection, CN3Base::s_CameraData.vp.Width, CN3Base::s_CameraData.vp.Height);
...
bool CGameProcMain::OnMouseLbtnDown(POINT ptCur, POINT ptPrev)
{
	...
	if(!s_pPlayer->IsDead() && VP_THIRD_PERSON == s_pEng->ViewPoint() && !s_pLocalInput->IsKeyDown(KM_MOVE_FOWARD))
	{
		__Vector3 vMovePoint;
		float fDist;
...
bool CGameProcMain::OnMouseRBtnPress(POINT ptCur, POINT ptPrev)
{
	...
	if(NULL == pNPC)
	{
		...
		if(false == this->MsgSend_RequestItemBundleOpen(pCorpse))
		{
			...
			if(	pShape && pShape == s_pPlayer->m_pObjectTarget && pShape->m_iEventID)
			{
...
{
	uint8_t	byBuff[10];	int		iOffset=0;
	float	fTime = CN3Base::TimeGet();							

	s_pSocket->MP_AddByte(byBuff, iOffset, WIZ_SPEEDHACK_CHECK);
	s_pSocket->MP_AddByte(byBuff, iOffset, bInit);
