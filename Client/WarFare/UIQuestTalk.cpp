...
void CUIQuestTalk::Open(Packet& pkt)
{
	...
	for(int i=0;i<MAX_STRING_TALK;i++)
	{
		...
		index = pkt.read<uint32_t>();
		_TBL_TABLE_QUEST_TALK* file_Tbl_QuestTalk = CGameBase::file_Tbl_QuestTalk.Find(index);
		if(file_Tbl_QuestTalk)
		{
			m_szTalk[i] = file_Tbl_QuestTalk->szTalk;
			m_iNumTalk++;
		}
	}
...
bool CUIQuestTalk::Load(HANDLE hFile)
{
	...
	__ASSERT(m_pTextTalk, "m_pTextTalk NULL UI Component!!!");
	
	m_pBtnOk	= (CN3UIButton*)(this->GetChildByID("btn_Ok_center"));		
	__ASSERT(m_pBtnOk, "m_pBtnOk NULL UI Component!!!");
