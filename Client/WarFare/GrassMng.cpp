...
void CGrassMng::Release()
{
	...
	CLogWriter::Write("CGrassMng::Release -> ZeroMemory"); 
	ZeroMemory(m_fChkRange,sizeof(m_fChkRange));

	for(int i=0;i<m_iFileMaxNum;i++)
	{
		CLogWriter::Write("CGrassMng::Release s_MngTex.Delete(%d)",m_txTexture[i]); 
...
{
	return (fPoX>fRange[GRASS_LEFT] && fPoX<fRange[GRASS_RIGHT]  && fPoY>fRange[GRASS_TOP] && fPoY<fRange[GRASS_BOTTOM]);
}

void CGrassMng::ChkTileRange(float fCamX,float fCamZ)
{
	if(IsInRect(fCamX,fCamZ,m_fChkRange)==true) return;
	...
	int iFineTile=0;
	FineNewTile(FineTile,iFineTile,fCompRange,fLargeRange);
		...
		for(int z=0; z<GRASS_TILENUM; ++z)
		{
			...
			if(IsInRect( vCenter.x,vCenter.z,fLargeRange )==false)
			{
				...
				usGrassInfo = ACT_WORLD->GetTerrainRef()->GetGrassAttr(
					...
					(FineTile[iTileCount+1]) / (int)TILE_SIZE);	

				vCenter.Set(FineTile[iTileCount],0.0f,FineTile[iTileCount+1]);	
				vCenter.y = ACT_WORLD->GetTerrainRef()->GetHeight(vCenter.x,vCenter.z);

				pBoard->SetInfo(vCenter,usGrassInfo);
