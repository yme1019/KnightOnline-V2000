...
void CBird::Tick()
{
	...
	__Vector3 vPos;
	 vPos.Set(sinf(m_fFactor1)*m_fRadius*cosf(m_fRadian), m_fRadiusY*(sinf(m_fFactorY1)+cosf(m_fFactorY2))/2.0f, sinf(m_fFactor2)*m_fRadius*sinf(m_fRadian));
...
int CBird::LoadBird(const std::string& szFN)
{
	...
	__ASSERT(m_pShape == NULL, "Bird memory leak 啊瓷己");
