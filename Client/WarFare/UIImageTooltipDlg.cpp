...
int	CUIImageTooltipDlg::CalcTooltipStringNumAndWrite(__IconItemSkill* spItem, bool bPrice, bool bBuy)
{
	...
	if ((!m_spItemBack) || (m_spItemBack->pItemBasic->dwID != spItem->pItemBasic->dwID) ||  (m_spItemBack->pItemExt->ItemIndex != spItem->pItemExt->ItemIndex) || (m_spItemBack->iDurability != spItem->iDurability) )
	{
	...
	if (!bPrice && (iIndex > MAX_TOOLTIP_COUNT-1))	 {__ASSERT(0, "Too Many Tooltip Item Info");	goto exceptions;}	\
		...
		if (m_pStr[iIndex])
		{
			...
			else if ( spItem->pItemBasic->szName != m_pStr[iIndex]->GetString() )
			{
				...
				{
					if(spItem->pItemExt->ItemIndex %10 != 0)
					{
						char szExtID[20];
						sprintf(szExtID,"(+%d)",spItem->pItemExt->ItemIndex %10);
						strtemp = szExtID;
					}
