...
void CUITargetBar::UpdateHP(int iHP, int iHPMax, bool bUpdateImmediately)
{
	...
	else m_pProgressHP->SetCurValue(iPercentage, 0.5f, 50.0f);	
	return;
}
...
bool CUITargetBar::Load(HANDLE hFile)
{
	...
	__ASSERT(m_pProgressHP, "m_pProgressHP NULL UI Component!!");
	
	m_pStringID = (CN3UIString*)GetChildByID("text_target");	
	__ASSERT(m_pStringID, "m_pStringID NULL UI Component!!");
	...
	__ASSERT(m_pProgressHP_posion, "m_pProgressHP_posion NULL UI Component!!");
	...
	__ASSERT(m_pProgressHP_curse, "m_pProgressHP_curse NULL UI Component!!");
	...
	__ASSERT(m_pProgressHP_lasting, "m_pProgressHP_lasting NULL UI Component!!");
