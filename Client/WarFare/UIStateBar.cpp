...
bool CUIStateBar::Load(HANDLE hFile)
{
	...
	__ASSERT(m_pText_Position, "m_pText_Position NULL UI Component!!");
	m_pProgress_HP =	(CN3UIProgress*)(this->GetChildByID("Progress_HP"));	
	__ASSERT(m_pProgress_HP, "m_pProgress_HP NULL UI Component!!");
	m_pProgress_MSP =	(CN3UIProgress*)(this->GetChildByID("Progress_MSP"));	
	__ASSERT(m_pProgress_MSP, "m_pProgress_MSP NULL UI Component!!");
	m_pProgress_ExpC =	(CN3UIProgress*)(this->GetChildByID("Progress_ExpC"));	
	__ASSERT(m_pProgress_ExpC, "m_pProgress_ExpC NULL UI Component!!");
	m_pProgress_ExpP =	(CN3UIProgress*)(this->GetChildByID("Progress_ExpP"));	
	__ASSERT(m_pProgress_ExpP, "m_pProgress_ExpP NULL UI Component!!");
	...
	__ASSERT(m_pProgress_HP_poison, "m_pProgress_HP_poison NULL UI Component!!");
	...
	__ASSERT(m_pProgress_HP_curse, "m_pProgress_HP_curse NULL UI Component!!");
	...
	__ASSERT(m_pText_HP, "m_pText_HP NULL UI Component!!");
	m_pText_MP = (CN3UIString*)GetChildByID("Text_MSP");
	__ASSERT(m_pText_MP, "m_pText_MP NULL UI Component!!");
	m_pText_Exp = (CN3UIString*)GetChildByID("Text_ExpP");
	__ASSERT(m_pText_Exp, "m_pText_Exp NULL UI Component!!");
	...
	__ASSERT(m_pGroup_MiniMap, "m_pGroup_MiniMap NULL UI Component!!");

	if(m_pGroup_MiniMap)
	{
		m_pGroup_MiniMap->SetVisible(false);
		m_pImage_Map =		(CN3UIImage*)(m_pGroup_MiniMap->GetChildByID("Img_MiniMap"));	
		__ASSERT(m_pImage_Map, "m_pImage_Map NULL UI Component!!");
		m_pBtn_ZoomIn =		(CN3UIButton*)(m_pGroup_MiniMap->GetChildByID("Btn_ZoomIn"));	
		__ASSERT(m_pBtn_ZoomIn, "m_pBtn_ZoomIn NULL UI Component!!");
		m_pBtn_ZoomOut =	(CN3UIButton*)(m_pGroup_MiniMap->GetChildByID("Btn_ZoomOut"));	
		__ASSERT(m_pBtn_ZoomOut, "m_pBtn_ZoomOut NULL UI Component!!");
	}
...
void CUIStateBar::UpdateExp(uint64_t iExp, uint64_t iExpNext, bool bUpdateImmediately)
{
	...
	if(iExpNext > 10)
	{
		...
		if(bUpdateImmediately) m_pProgress_ExpC->SetCurValue(iPercentage2);	 
		else m_pProgress_ExpC->SetCurValue(iPercentage2, 0.7f, 50.0f);
	}
	else
	{
		m_pProgress_ExpC->SetCurValue(0);	
	}

	if(bUpdateImmediately) m_pProgress_ExpP->SetCurValue(iPercentage);
...
void CUIStateBar::UpdateMSP(int iMSP, int iMSPMax, bool bUpdateImmediately)
{
	...
	if(bUpdateImmediately) m_pProgress_MSP->SetCurValue(iPercentage);
...
void CUIStateBar::TickMiniMap()
{
	...
	for (int i = 0; i < 6; i++) m_vArrows[i] *= mtxRot;
}
...
void CUIStateBar::TickMagicIcon()
{
	...
	_TB_TABLE_UPC_SKILL* pRemoveSkill = NULL;
	...
	for (int i = 0; i < cnt; i++, it++)
	{
		...
		if (pMagicImg->fDuration <= 0.0f)
		{
			...
			pRemoveSkill = CGameBase::file_Tbl_Skill.Find(pMagicImg->dwSkillID);
			break;
		}
	...
	else if(dwMsg==UIMSG_ICON_DBLCLK)
	{
		...
		for(it = m_pMagic.begin(); it!=ite; it++)
		{
			...
			if(pSender==pMagicImg->pIcon)
			{
				...
				_TB_TABLE_UPC_SKILL* pSkill = CGameBase::file_Tbl_Skill.Find(MagicID);
...
void CUIStateBar::AddMagic(_TB_TABLE_UPC_SKILL* pSkill, float fDuration)
{
	std::vector<char> buffer(256, NULL);
	sprintf(&buffer[0],	"UI\\skillicon_%.2d_%d.dxt", pSkill->SkllNum%100, pSkill->SkllNum/100);

	__DurationMagicImg* pMagicImg = new __DurationMagicImg;
	pMagicImg->fDuration = fDuration;
	pMagicImg->pIcon = new CN3UIDBCLButton;
	pMagicImg->dwSkillID = pSkill->SkllNum;
...
void CUIStateBar::DelMagic(_TB_TABLE_UPC_SKILL* pSkill)
{
	std::vector<char> buffer(256, NULL);
	sprintf(&buffer[0],	"UI\\skillicon_%.2d_%d.dxt", pSkill->SkllNum %100, pSkill->SkllNum /100);
...
uint32_t CUIStateBar::MouseProc(uint32_t dwFlags, const POINT &ptCur, const POINT &ptOld)
{
	...
	for(int i=0;i<cnt;i++,it++)
	{
		...
		dwRet |= pMagicImg->pIcon->MouseProc(CGameProcedure::s_pLocalInput->MouseGetFlag(), CGameProcedure::s_pLocalInput->MouseGetPos(), CGameProcedure::s_pLocalInput->MouseGetPosOld());
	}
