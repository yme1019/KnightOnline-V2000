...
void CGameProcCharacterSelect::Tick()
{
	...
	if(s_pUIMgr->m_bDoneSomething == false && s_pUIMgr->EnableOperation())
	{
		...
		int nMFlags = s_pLocalInput->MouseGetFlag();				
...
NowRotating:
	...
	for(int i = 0; i < 8; i++) s_pEng->s_lpD3DDev->LightEnable(i, FALSE);
...
void CGameProcCharacterSelect::AddChr(e_ChrPos eCP, __CharacterSelectInfo* pInfo)
{
	...
	if(pItemUpper && pItemUpper->byIsRobeType) m_pChrs[iPosIndex]->PartSet(PART_POS_LOWER, ""); 
	else this->AddChrPart(iPosIndex, pLooks, PART_POS_LOWER, pInfo->dwItemLower, pInfo->iItemLowerDurability);
...
void CGameProcCharacterSelect::AddChrPart(int iPosIndex, const __TABLE_PLAYER_LOOKS* pLooks, e_PartPosition ePartPos, uint32_t dwItemID, int iItemDurability)
{
...
bool CGameProcCharacterSelect::MsgRecv_CharacterSelect(Packet& pkt) // virtual
{
	...
	if(bSuccess) this->CharacterSelect();
...
void CGameProcCharacterSelect::MsgSend_DeleteChr(const std::string& szKey)
{
	...
	uint8_t byBuff[64];
	int iOffset = 0;
	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_DEL_CHAR);
	CAPISocket::MP_AddByte(byBuff, iOffset, (uint8_t)iIndex);				
	CAPISocket::MP_AddShort(byBuff, iOffset, (int16_t)m_InfoChrs[iIndex].szID.size());		
	CAPISocket::MP_AddString(byBuff, iOffset, m_InfoChrs[iIndex].szID); 
	CAPISocket::MP_AddShort(byBuff, iOffset, (int16_t)szKey.size());				
...
void CGameProcCharacterSelect::MsgRecv_AllCharacterInfo(Packet& pkt)
{
	...
	if(0x1 == iResult)
	{
		for(int i = 0; i < MAX_AVAILABLE_CHARACTER; i++)
		{
			...
			pkt.readString(m_InfoChrs[i].szID, iIDLength);

			m_InfoChrs[i].eRace			= (e_Race)(pkt.read<uint8_t>()); 
			m_InfoChrs[i].eClass		= (e_Class)(pkt.read<int16_t>());
			m_InfoChrs[i].iLevel		= pkt.read<uint8_t>();
			m_InfoChrs[i].iFace			= pkt.read<uint8_t>(); 
			m_InfoChrs[i].iHair			= pkt.read<uint8_t>(); 
			m_InfoChrs[i].iZone			= pkt.read<uint8_t>();

			m_InfoChrs[i].dwItemHelmet				= pkt.read<uint32_t>();
			m_InfoChrs[i].iItemHelmetDurability		= pkt.read<int16_t>(); 
			m_InfoChrs[i].dwItemUpper				= pkt.read<uint32_t>(); 
			m_InfoChrs[i].iItemUpperDurability		= pkt.read<int16_t>(); 
			m_InfoChrs[i].dwItemCloak				= pkt.read<uint32_t>(); 
			m_InfoChrs[i].iItemCloakDurability		= pkt.read<int16_t>(); 
			...
			m_InfoChrs[i].dwItemLower				= pkt.read<uint32_t>();
			m_InfoChrs[i].iItemLowerDurability		= pkt.read<int16_t>();
			m_InfoChrs[i].dwItemGloves				= pkt.read<uint32_t>();
			m_InfoChrs[i].iItemGlovesDurability		= pkt.read<int16_t>();
			m_InfoChrs[i].dwItemShoes				= pkt.read<uint32_t>();
			m_InfoChrs[i].iItemShoesDurability		= pkt.read<int16_t>();
		}
...
void CGameProcCharacterSelect::MsgSend_RequestAllCharacterInfo()
{
	uint8_t byBuff[4];
	int iOffset = 0;
	CAPISocket::MP_AddByte(byBuff, iOffset, WIZ_ALLCHAR_INFO_REQ);
	s_pSocket->Send(byBuff, iOffset);
}
...
void CGameProcCharacterSelect::MsgSend_CharacterSelect() // virtual
{
	...
	s_pUIMgr->EnableOperationSet(false); // UI 
}
