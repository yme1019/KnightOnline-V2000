...
bool CUINationSelectDlg::Load(HANDLE hFile)
{
	...
	m_pBtnKarus = this->GetChildByID("btn_karus_selection");	
	__ASSERT(m_pBtnKarus, "m_pBtnKarus NULL UI Component!!");

	m_pBtnElmorad = this->GetChildByID("btn_elmo_selection");	
	__ASSERT(m_pBtnElmorad, "m_pBtnElmorad NULL UI Component!!"); 	

	m_pBtnBack = this->GetChildByID("btn_back");	
	__ASSERT(m_pBtnBack, "m_pBtnBack NULL UI Component!!");
