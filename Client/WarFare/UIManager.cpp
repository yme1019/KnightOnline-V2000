...
uint32_t CUIManager::MouseProc(uint32_t dwFlags, const POINT& ptCur, const POINT& ptOld)
{
	...
	if (s_pTooltipCtrl)	s_pTooltipCtrl->MouseProc(dwFlags, ptCur, ptOld);
...
//	if(UI_MOUSE_LBCLICK & dwFlags) m_pUIFocused = NULL; 
...
void CUIManager::Render()
{
	...
	*/
	////////////////////////////////////////////////////////
#ifdef _DEBUG
	CDFont* m_pDFont = new CDFont("굴림", 10);	
...
CN3UIBase* CUIManager::GetEnableFocusTopUI(bool bVisible)
{
	...
	for(; it != itEnd; it++)
	{
		...
		if(pUI && pUI->IsVisible() &&	
			!(pUI->GetStyle() & UISTYLE_FOCUS_UNABLE) && 
			!(pUI->GetStyle() & UISTYLE_HIDE_UNABLE)) 
			return pUI;
	}
