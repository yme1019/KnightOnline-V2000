...
{
	_TBL_TABLE_FX* pFX = s_pTbl_FXSource.Find(FXID);
	if(!pFX) return;

	char buff[MAX_PATH];
	sprintf(buff, pFX->fxNamefxb.c_str());
	_strlwr(buff);
	std::string strTmp = buff;
	
	stlMAP_BUNDLEORIGIN_IT itOrigin = m_OriginBundle.find(strTmp);	
	if(itOrigin != m_OriginBundle.end())
	{
		LPFXBUNDLEORIGIN pSrc = itOrigin->second;
...
void CN3FXMgr::TriggerBundle(int SourceID, int SourceJoint, int FXID, __Vector3 TargetPos, int idx, int MoveType, uint32_t iVer)
{
	...
	else
	{
		LPFXBUNDLEORIGIN pSrc = new FXBUNDLEORIGIN;
		pSrc->pBundle = new CN3FXBundleGame;
		pSrc->pBundle->LoadFromFile(pFX->fxNamefxb, iVer);
	...
	while(it!=m_ListBundle.end())
	{
		...
		if(pBundle->m_iMoveType != FX_BUNDLE_MOVE_NONE && pBundle->m_dwState==FX_BUNDLE_STATE_LIVE)
		{
			...
			if(s_pPlayer->IDNumber() == pBundle->m_iSourceID)
				...
				if( s_pPlayer->IDNumber()==pBundle->m_iTargetID && s_pOPMgr->UPCGetByID(pBundle->m_iSourceID, true)==NULL) dwToMe = 2;
			...
			if(dwToMe == 1 || dwToMe == 2)
			{
				...
				if(bCol == false && true == ACT_WORLD->CheckCollisionWithShape(pBundle->m_vPos, pBundle->m_vDir, pBundle->m_fVelocity * CN3Base::s_fSecPerFrm, &vCol))
				{
					bCol = true;
					pBundle->m_vPos = vCol;
