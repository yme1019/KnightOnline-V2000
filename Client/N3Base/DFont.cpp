...
CDFont::CDFont(const std::string& szFontName, uint32_t dwHeight, uint32_t dwFlags)
{
	if(0 == s_iInstanceCount)
	{
		...
		HFONT hFont	= CreateFont( 0, 0, 0, 0, 0, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, VARIABLE_PITCH, "굴림");
		if(hFont)
		{
...
HRESULT CDFont::SetFont(const std::string& szFontName, uint32_t dwHeight, uint32_t dwFlags)
{
    ...
    m_hFont	= CreateFont(nHeight, 0, 0, 0, dwBold, dwItalic, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, VARIABLE_PITCH, m_szFontName.c_str());
    if( NULL== m_hFont )
	{
...
HRESULT CDFont::SetText(const std::string& szText, uint32_t dwFlags)
{
	...
	if (NULL == m_pTexture)
	{
		...
		hr = m_pd3dDevice->CreateTexture( m_dwTexWidth, m_dwTexHeight, iMipMapCount, 0, D3DFMT_A4R4G4B4, D3DPOOL_MANAGED, &m_pTexture, NULL );
		if( FAILED(hr) )
			return hr;
	}
	...
	while(iCount<iStrLen)
	{
		if ('\n' == szText[iCount])		
		{
			...
			if (sx != x)
			{
				...
				__ASSERT(dwNumTriangles+2 < MAX_NUM_VERTICES, "??");		// Vertex buffer�
...
void CDFont::Make3DVertex(const int iFontHeight, const std::string& szText, uint32_t dwFlags)
{
	...
	while(iCount<iStrLen)
	{
		...
		else if (0x80 & szText[iCount])	// 2BYTE
		{
			memcpy(szTempChar, &(szText[iCount]), 2);
			iCount += 2;
			szTempChar[2] = 0x00;
