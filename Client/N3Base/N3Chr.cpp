...
void CN3Chr::Tick(float fFrm)
{
	...
	m_nLOD = MAX_CHR_LOD;
	float fLOD = fDist * s_CameraData.fFOV / (m_fRadius * m_vScale.x);
	for(int i = 0; i < MAX_CHR_LOD; i++)
	{
		if(fLOD < CHR_LOD_CALCULATION_VALUES[0][i])
		{
			m_nLOD = i;
			break;
		}
	...
	if(	s_CameraData.IsOutOfFrustum(m_vPos, fOffset) &&
		s_CameraData.IsOutOfFrustum(vPos2, fOffset) )
	{
		m_nLOD = -1;
		return;
	}

	m_FrmCtrl.fFrmPrev = m_FrmCtrl.fFrmCur;
