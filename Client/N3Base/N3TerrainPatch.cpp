...
{
	if(m_ti_LBPoint.x < 0 || m_ti_LBPoint.y < 0 || m_ti_LBPoint.x >= (m_pRefTerrain->m_ti_MapSize-1) || m_ti_LBPoint.y >= (m_pRefTerrain->m_ti_MapSize-1)
	)
	{
		m_bIsRender = FALSE;		
	}
	...
	if(m_iLevel==1)
	{
		...
		for(ix=0; ix<PATCH_TILE_SIZE;ix++)
		{
			for(iz=0; iz<PATCH_TILE_SIZE;iz++)
			{
				...
				if((ix+iz)%2==0)
				{
					pVertices[VertexIdx].Set( (float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx,tz).fHeight, (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, u1[0], v1[0], u2[0], v2[0]);
					pVertices[VertexIdx+1].Set( (float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx,tz+1).fHeight, (float)((tz+1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, u1[1], v1[1], u2[1], v2[1]);
					pVertices[VertexIdx+2].Set( (float)((tx+1) * TILE_SIZE), m_pRefTerrain->GetMapData(tx+1,tz+1).fHeight, (float)((tz+1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, u1[2], v1[2], u2[2], v2[2]);
					pVertices[VertexIdx+3].Set( (float)((tx+1) * TILE_SIZE), m_pRefTerrain->GetMapData(tx+1,tz).fHeight, (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, u1[3], v1[3], u2[3], v2[3]);
					VertexIdx+=4;
					TileCount++;
					...
					if(m_pRefTerrain->GetLightMap(tx, tz))
					{
						...
						pLightMapVertices[VBIndx].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx,tz).fHeight, (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, suv, euv);
						pLightMapVertices[VBIndx+1].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx,tz+1).fHeight, (float)((tz+1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, suv, suv);
						pLightMapVertices[VBIndx+2].Set((float)((tx+1) * TILE_SIZE), m_pRefTerrain->GetMapData(tx+1,tz+1).fHeight, (float)((tz+1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, euv, suv);
						pLightMapVertices[VBIndx+3].Set((float)((tx+1) * TILE_SIZE), m_pRefTerrain->GetMapData(tx+1,tz).fHeight, (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, euv, euv);		
						m_NumLightMapTex++;			
					}
				}
				else
				{
					...
					if(m_pRefTerrain->GetLightMap(tx, tz))
					{
						...
						pLightMapVertices[VBIndx].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx,tz+1).fHeight, (float)((tz+1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, suv, suv);
						pLightMapVertices[VBIndx+1].Set((float)((tx+1) * TILE_SIZE), m_pRefTerrain->GetMapData(tx+1,tz+1).fHeight, (float)((tz+1) * TILE_SIZE), 0.0f, 1.0f, 0.0f, euv, suv);
						pLightMapVertices[VBIndx+2].Set((float)((tx+1) * TILE_SIZE), m_pRefTerrain->GetMapData(tx+1,tz).fHeight, (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, euv, euv);
						pLightMapVertices[VBIndx+3].Set((float)(tx * TILE_SIZE), m_pRefTerrain->GetMapData(tx,tz).fHeight, (float)(tz * TILE_SIZE), 0.0f, 1.0f, 0.0f, suv, euv);						
						m_NumLightMapTex++;			
					}
				}
			}
...
void CN3TerrainPatch::Render()
{
	...
	if(m_iLevel==1)
	{
		...
		for(int i=0; i<TotalTile; i++)
		{
			...
			if( (!m_pRefTerrain->m_bAvailableTile) && m_pTileTexIndx[0][i] < m_pRefTerrain->m_NumTileTex && m_pTileTexIndx[1][i] < m_pRefTerrain->m_NumTileTex )
			{
